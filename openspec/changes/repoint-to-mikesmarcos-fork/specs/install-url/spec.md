...
Define the invariant that every URL the project publishes — the install-script source (`REPO_GIT_URL`), the README badges, and the README install/uninstall snippets — points at the canonical Git URL of this project (`https://github.com/mikesmarcos/free-claude-code`), and that a guard test fails if the URL drifts. The capability exists so a maintainer cannot silently re-point the project back at a stale fork (or at the upstream) without breaking the contract.
